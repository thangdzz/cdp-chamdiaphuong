...
import { isLocationVerified, locationSourceLabel, locationOf } from "@/lib/placeLocation";
...
export function PlaceLocationEditor({ place }) {
  ...
  const verified = isLocationVerified({ ...place, coordinates });
  ...
  return (
    <fieldset className="mt-3 rounded-lg border border-zinc-200 bg-white p-2">
      ...
      <input type="hidden" name="placeLocationJson" value={JSON.stringify(coordinates ?? null)} />
